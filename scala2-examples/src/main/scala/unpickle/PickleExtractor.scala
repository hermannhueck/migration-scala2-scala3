...
object PickleExtractor {
  ...
  def process(input: Path, output: Path): Unit = {
    ...
    try {
      ...
      s"--> creating directory: ${outputPath.root}" pipe println
      ...
      val visitor = new SimpleFileVisitor[Path] {
        override def preVisitDirectory(dir: Path, attrs: BasicFileAttributes): FileVisitResult = {
          if (dir != root) {
            ...
            s"--> creating directory: $outputDir" pipe println
          ...
          if (file.getFileName.toString.endsWith(".class")) {
            try {
              stripClassFile(Files.readAllBytes(file)) match {
                case Class(out) =>
                  val outPath = outputPath.root.resolve(root.relativize(file).toString)
                  s"--> writing class byte code to: $outPath" pipe println
                  Files.write(outPath, out)
                case Pickle(out) =>
                  val outPath = outputPath.root.resolve(root.relativize(file).toString.replaceAll(".class$", ".sig"))
                  s"--> writing pickled signatures to: $outPath" pipe println
                  Files.write(outPath, out)
                case Skip =>
              }
            } catch {
              case ex: RuntimeException =>
                throw new RuntimeException("While parsing: " + file + " in " + inputPath, ex)
  ...
  def stripClassFile(classfile: Array[Byte]): OutputFile = {
    ...
    s"\n===> input.name = ${input.name}" pipe println
    // s"===> input.access = ${input.access}" pipe println
    // s"===> input.version = ${input.version}" pipe println

    var foundScalaSig = false

    def isScalaAnnotation(desc: String) =
      (desc == "Lscala/reflect/ScalaSignature;" || desc == "Lscala/reflect/ScalaLongSignature;") && {
        foundScalaSig = true
    ...
    if (input.visibleAnnotations != null) {
      // s"===> input.visibleAnnotations = ${input.visibleAnnotations}" pipe println
      ...
      output.visibleAnnotations = input
        .visibleAnnotations
        .asScala
        .filter(node =>
          isScalaAnnotation(node.desc) && {
            true
          }
        )
        .asJava
    ...
    if (input.attrs != null) {
      s"===> input.attrs = ${input.attrs}" pipe println
      output.attrs = input
        .attrs
        .asScala
        .filter(attr => {
          s"===> attr = ${attr}" pipe println
          s"=====> attr.`type` = ${attr.`type`}" pipe println
          (attr.`type` == "Scala" || attr.`type` == "ScalaSig") && {
            foundScalaAttr = true;
            true
          }
        })
    ...
    if (isScalaRaw) {
      s"--> no raw Scala data found in class: ${input.name}" pipe println
      Skip
    } else {
      if (pickleData == null) {
        s"--> no pickled data found in class: ${input.name}" pipe println
        output = input
        output.accept(writer)
        Class(writer.toByteArray)
      } else {
        s"--> found pickled data in class: ${input.name}" pipe println
        output.accept(writer)
        Pickle(pickleData)
      }
    }
